...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // dogsJuliaCorrect = dogsJulia.slice(1, -2);

  // because of creating shallow copy task
  dogsJuliaCorrect = dogsJulia.slice();
  dogsJuliaCorrect.splice(0, 1);
  dogsJuliaCorrect.splice(-2);

  // 2
  allDogs = dogsJuliaCorrect.concat(dogsKate);
...
//   .filter(dog => dog.breed === 'Husky')
//   .reduce((avg, dog, _, arr) => avg + dog.averageWeight / arr.length);
...
// const heaviestFetchWeight = breeds
//   .filter(dog => dog.activities.includes('fetch'))
//   .reduce((max, dog) => (dog.averageWeight > max ? dog.averageWeight : max), 0);
...
//   exact: exactDogs,
//   tooMuch: tooMuchDogs,
//   tooLittle: tooLittleDogs,
// } = Object.groupBy(dogs, dog => {
//   if (dog.curFood > dog.recFood * 1.1) return 'tooMuch';
//   else if (dog.curFood < dog.recFood * 0.9) return 'tooLittle';
//   else return 'exact';
// });
...
const {
  ...
  if (dog.curFood > dog.recFood) return 'tooMuch';
  else if (dog.curFood < dog.recFood) return 'tooLittle';
  else return 'exact';
});

console.log(exactDogs, tooMuchDogs, tooLittleDogs);
